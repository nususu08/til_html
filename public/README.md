...
- Image preview
- indent - rainbow
- Korean Languge Pack for... (VSCode 재실행 후 확인)
- Live Server
...
```
jpg, png(추천), gif, svg(추천), webp(추천 : 용량 최소화) 등
```
- `프로젝트명/public/css`
...
```
관리도구 버튼 → Settings → 검색창 → default:windows → Git Bash 선택
```
...
- 옛날에는 브랜치명이 `master 라고 생성`이 되었음. (노예라는 어감)
- 현대에는 브랜치명이 `main` 으로 생성해야함.

```bash
git config --global init.defaultBranch main
```

### 1-4. Enter 키에 의한 줄내림 통일하기(Linux, Macm Window)
